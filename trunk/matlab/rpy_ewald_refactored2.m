...
% compute reciprocal mesh constants
vsize = ((2*nk+1)^3 - 1) / 2;
k_array = zeros(1,vsize);
m2_array = zeros(1,vsize);
kvec_array = zeros(3,vsize);
...
for z = -nk:nk
  ...
  k = norm(kvec);
  k_array(ind) = k;
  m2_array(ind) = scalar_rpy_ewald_recip(k, xi);
  kvec_array(:,ind) = kvec;
...
for i = 1:npos
  ...
  for j = i:npos
    ...
    for ind = 1:vsize
      ...
      m2 = m2_array(ind);
      kvec = kvec_array(:,ind);
      temp = temp + 2 * m2*cos(kvec'*rvec)*(eye3 - kvec*kvec'/(k*k));
    end
